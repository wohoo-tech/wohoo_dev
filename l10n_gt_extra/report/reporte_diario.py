...
class ReporteDiario(models.AbstractModel):
    ...
    def lineas(self, datos):
        ...
        if datos['agrupado_por_dia']:
            ...
            self.env.cr.execute(f"""select a.id, a.code as codigo, a.name as cuenta, t.include_initial_balance as balance_inicial, sum(l.debit) as debe, sum(l.credit) as haber 
            	from account_move_line l join account_account a on(l.account_id = a.id)
                where l.parent_state = 'posted' and a.id in ({accounts_str}) and a.account_type and l.date >= %s and l.date <= %s group by a.id, a.code, a.name,t.include_initial_balance ORDER BY a.code""",
            (datos['fecha_desde'], datos['fecha_hasta']))
